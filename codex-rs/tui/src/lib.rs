...
async fn run_ratatui_app(
    ...
    let remote_project_trust =
        if uses_remote_workspace && let Some(remote_cwd) = remote_cwd_override.as_deref() {
            match config_update::read_remote_project_trust(
                app_server_session.request_handle(),
                remote_cwd,
            )
            .await
            {
                Ok(remote_project_trust) => remote_project_trust,
                Err(err) => {
    ...
    let (login_status, mut startup_account) = if workload_identity_selected {
        ...
        let login_status = startup_draft
            ...
            .await;
        match login_status {
            Ok(Ok((login_status, account))) => (login_status, account),
            Ok(Err(err)) => {
...
async fn get_login_status(
    app_server: &mut AppServerSession,
    config: &Config,
) -> color_eyre::Result<(LoginStatus, Option<GetAccountResponse>)> {
    if !config.model_provider.requires_openai_auth {
        return Ok((LoginStatus::NotAuthenticated, None));
    }
...
"#
            ...
            assert_eq!(
                account.as_ref().map(|account| account.requires_openai_auth),
                requires_openai_auth.then_some(true)
            );
            assert_eq!(
                should_show_login_screen(login_status, requires_openai_auth),
                requires_openai_auth
