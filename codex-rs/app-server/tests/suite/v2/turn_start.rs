...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    let model = cache["models"][0]["slug"]
        .as_str()
        .expect("models cache should contain a model slug");
    MockResponsesConfig::new(&server.uri())
        .enable_feature(Feature::Personality)
        .with_model(model)
        .with_root_config("model_context_window = 100")
        .write(codex_home.path())?;
    ...
    assert_eq!(
        warning.message,
        "Exceeded skills context budget. All skill descriptions were removed and 8 additional skills were not included in the model-visible skills list."
    );
