...
async fn cli_main(
        ...
        && root_endpoint != agents_endpoint
    {
        ...
        anyhow::bail!("`{command_name} agents` received conflicting remote server endpoints");
    }
    ...
    match subcommand {
        None | Some(Subcommand::Agents(_)) => {
            ...
            if open_agents_overview {
                ...
                if interactive.prompt.is_some() || !interactive.images.is_empty() {
                    anyhow::bail!(
                        "`{command_name} agents` does not accept an initial prompt or images"
                    );
                }
                ...
                {
                    anyhow::bail!(
                        "`{command_name} agents` cannot apply local provider or additional-directory overrides to a remote server"
                    );
                }
                if is_workload_identity_selected() {
                    anyhow::bail!(
                        "`{command_name} agents` is unavailable while workload identity is active"
                    );
                }
                if root_remote.is_none() {
                    ...
                    #[cfg(not(unix))]
                    anyhow::bail!("`{command_name} agents` requires `--remote` on this platform");
                }
        ...
        Some(Subcommand::McpServer(McpServerCommand { strict_config })) => {
            let command_name = product_command_name();
            eprintln!(
                "warning: `{command_name} mcp-server` is deprecated and will be removed in a future release."
            );
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => {
            let command = product_command_name();
            anyhow::bail!(
                "--profile only applies to runtime commands and `{command} mcp`: `{command}`, `{command} exec`, `{command} review`, `{command} resume`, `{command} queue`, `{command} archive`, `{command} delete`, `{command} unarchive`, `{command} fork`, `{command} mcp`, `{command} sandbox`, and `{command} debug prompt-input`."
            )
        }
    }
}
