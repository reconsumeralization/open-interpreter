...
pub(super) async fn updates_check(config: &Config) -> DoctorCheck {
    ...
    match fetch_latest_version(&client, &install_context, Product::current()).await {
        Ok(latest_version) => {
...
async fn fetch_latest_version(
    ...
    match latest_version_source(context, product) {
        LatestVersionSource::Homebrew => fetch_homebrew_cask_version(client).await,
        LatestVersionSource::GitHub(url) => fetch_latest_github_release_version(client, url).await,
    }
}
