...
#[cfg(target_os = "windows")]
const WINDOWS_SETUP_REMEDIATION: &str = "run the sandbox setup command with --elevated --user <end-user> --codex-home <authoritative-home> from an elevated shell";
                ...
                if let Some(report) = report {
                    let remediation = match report.code {
                        ...
                        | SetupErrorCode::HelperReadAclHelperSpawnFailed => {
                            "repair the installed agent helpers or ask IT to allow their execution"
                        }
                        ...
                        | SetupErrorCode::HelperSandboxLockFailed => {
                            "rerun elevated setup for the authoritative agent home or ask IT"
                        }
                        ...
                        | SetupErrorCode::HelperFirewallRuleVerifyFailed => {
                            "ask IT to allow the agent sandbox rules in managed Windows Firewall policy"
                        }
                        SetupErrorCode::OrchestratorPayloadSerializeFailed
                        | SetupErrorCode::HelperRequestArgsFailed
                        | SetupErrorCode::HelperUnknownError => {
                            "repair or reinstall the agent CLI from an approved distribution"
                        }
                    };
                    check
                        .details
