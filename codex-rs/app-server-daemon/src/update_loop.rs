...
#[cfg(windows)]
const OPEN_INTERPRETER_INSTALL_URL: &str = "https://www.openinterpreter.com/install.ps1";
...
async fn install_latest_standalone(http: &impl InstallerHttp) -> Result<()> {
    ...
    #[cfg(unix)]
    let script = fetch_installer_script(http, product.installer_url()).await?;
    #[cfg(windows)]
    let install_url = match product {
        Product::Codex => INSTALL_URL,
        Product::OpenInterpreter => OPEN_INTERPRETER_INSTALL_URL,
    };
    #[cfg(windows)]
    let script = fetch_installer_script(http, install_url).await?;

    #[cfg(unix)]
    let mut command = {
        let mut command = Command::new("/bin/sh");
        command.arg("-s");
        command.envs(product.installer_env().iter().copied());
        command
    };
    #[cfg(windows)]
    let mut command = {
...
async fn fetch_installer_script(http: &impl InstallerHttp, install_url: &str) -> Result<Vec<u8>> {
