    ...
    loop {
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if !needs_follow_up {
                    if turn_context.config.harness.as_deref() == Some("deepseek-tui")
                    ...
                    if turn_context.config.harness.as_deref() == Some("claude-code-bare")
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let in_flight_ordering = if turn_context.config.harness.as_deref() == Some("zcode") {
