...
#[tokio::test]
async fn cli_wire_api_override_changes_only_the_effective_provider_transport() -> std::io::Result<()>
{
    let codex_home = TempDir::new()?;
    let baseline = ConfigBuilder::without_managed_config_for_tests()
        .codex_home(codex_home.path().to_path_buf())
        .fallback_cwd(Some(codex_home.path().to_path_buf()))
        .build()
        .await?;
    let overridden = ConfigBuilder::without_managed_config_for_tests()
        .codex_home(codex_home.path().to_path_buf())
        .fallback_cwd(Some(codex_home.path().to_path_buf()))
    ...
    assert_eq!(baseline.model_provider.wire_api, WireApi::Responses);
    assert_eq!(overridden.model_provider.wire_api, WireApi::Chat);
    assert_eq!(overridden.model_provider_id, baseline.model_provider_id);
    assert_eq!(
        overridden.model_provider.base_url,
        baseline.model_provider.base_url
    );
    assert_eq!(
        overridden.model_provider.env_key,
        baseline.model_provider.env_key
    );
    Ok(())
}
